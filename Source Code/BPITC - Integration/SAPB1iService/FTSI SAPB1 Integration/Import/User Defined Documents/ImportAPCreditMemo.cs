            ...
            try
            {
                ...
                foreach (DataRow oDataRow in dsProcessData.Tables[0].Rows)
                {
                    ...
                    try
                    {
                        ...
                        DataSet dsBusinessObject = SQLSettings.getDataFromMySQL(string.Format("CALL FTSI_IMPORT_AP_CREDIT_MEMO('{0}', '{1}')", strId, strDocType));

                        // Rename DataTables.
                        // NOTE: Make sure to rename DataTable because the names will be used as TAGS in XML file.
                        dsBusinessObject.Tables[0].TableName = "ORPC";
                        dsBusinessObject.Tables[1].TableName = "RPC1";
                        dsBusinessObject.Tables[2].TableName = "RPC5";

                        // Process XML File Creation
                        strXmlPath = GenerateFilePath(dsBusinessObject.Tables["ORPC"].Rows[0]["U_RefNum"].ToString());
