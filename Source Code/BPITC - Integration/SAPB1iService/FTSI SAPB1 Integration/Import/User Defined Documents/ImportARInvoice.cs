...
namespace FTSISAPB1iService
{
    public class ImportARInvoice
    {
        ...
        public static void _ImportARInvoice()
        {
            ...
            int intDPDocEntry = 0;
            string strDPU_RefNum = string.Empty;
            //string strJrnlMemo = string.Empty;
                ...
                foreach (DataRow oDataRow in dsProcessData.Tables[0].Rows)
                {
                    ...
                    try
                    {
                        ...
                        if (!GlobalFunction.checkRefNum(strU_RefNum, GlobalVariable.strTableHeader))
                        {
                            ...
                            DataSet dsBusinessObject = SQLSettings.getDataFromMySQL(string.Format("CALL FTSI_IMPORT_AR_INVOICE('{0}', '{1}')", strId, strDocType));
                            ...
                            if (strDocType == "I")
                            {
                                ...
                                dsBusinessObject.Tables[3].TableName = "INV9";

                                //strJrnlMemo = dsBusinessObject.Tables[0].Rows[0]["JrnlMemo"].ToString();

                                for (int i = 0; i < dsBusinessObject.Tables["INV9"].Rows.Count; i++)
                                {
                                    strDPU_RefNum = dsBusinessObject.Tables[3].Rows[i]["U_DPRefNum"].ToString();
                                   
                                    string qrydpDocEntry = $"SELECT DocEntry FROM ODPI WHERE U_RefNum = '{strDPU_RefNum}'";
                                    oRecordset.DoQuery(qrydpDocEntry);
                                    intDPDocEntry = (int)oRecordset.Fields.Item("DocEntry").Value;
                                    dsBusinessObject.Tables["INV9"].Rows[i]["DocEntry"] = intDPDocEntry;
                                }
                            }
                            else
                            {
                            ...
                            if (ImportDocumentsXML.importTempXMLDocument(strXmlPath, strId))
                            {
                                ...
                                //Update Retirement
                                //GlobalFunction.hasRetirement(strPostDocNum, strJrnlMemo);
