...
namespace FTSISAPB1iService
{
    class ImportIncomingPayment
    {
        ...
        public static void _ImportIncomingPayment()
        {
            ...
            string strInvType = string.Empty;
            ...
            try
            {
                ...
                foreach (DataRow oDataRow in dsProcessData.Tables[0].Rows)
                {
                    ...
                    strInvType = oDataRow["InvType"].ToString();
                    ...
                    try
                    {
                        if (strCancel == "Y") // process is for cancellation
                        {
                            ...
                            oPayment = (SAPbobsCOM.Payments)GlobalVariable.oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.oIncomingPayments);

                            string query = $"SELECT DocEntry, Canceled FROM ORCT WHERE U_RefNum = '{strU_RefNum}'";
                            ...
                            try
                            {
                                if (oPayment.GetByKey(docEntry)) // docEntry
                                {
                                    oPayment.Cancel(); 
                                    oPayment.Update();
                                    ...
                                    SQLSettings.executeQuery(string.Format("UPDATE {0} SET Canceled = 'S', IntegrationStatus = 'S' , IntegrationMessage = \"Successfully Cancelled\" WHERE U_RefNum = '{1}'", strMySQLTable, strU_RefNum));
                                }
                                else
                                {
                                    ...
                                    // Update Staging DB
                                    SQLSettings.executeQuery(string.Format("UPDATE {0} SET Canceled = 'E', IntegrationStatus = 'E', IntegrationMessage = \"{1}\" WHERE Id = '{2}'", strMySQLTable, GlobalVariable.strErrMsg.Replace("\\", "").Replace("\"", "'"), strId));
                                }
                            }
                            catch (Exception ex)
                            {
                        ...
                        else  // process is for posting incoming payment
                        {
                            // SystemFunction.errorAppend(string.Format("CALL FTSI_IMPORT_INCOMING_PAYMENT('{0}', '{1}')", strId, strInvType));

                            // Get Document Header and Line Details
                            DataSet dsBusinessObject = SQLSettings.getDataFromMySQL(string.Format("CALL FTSI_IMPORT_INCOMING_PAYMENT('{0}', {1})", strId, strInvType));
