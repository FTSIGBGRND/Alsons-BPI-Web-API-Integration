...
namespace FTSISAPB1iService
{
    class GlobalFunction
    {
        ...
        public static void hasRetirement(string strDocNum, string strReference)
        {   
            int intDocEntry;
            DateTime dteStart;
            ...
            try
            {
                ...
                oRecordset = (SAPbobsCOM.Recordset)GlobalVariable.oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);
                oRecordset.DoQuery($"SELECT * FROM ORTI WHERE BaseRef = '{strDocNum}'");

                intDocEntry = Convert.ToInt32(oRecordset.Fields.Item("DocEntry").Value);
                ...
                if (oRecordset.RecordCount > 0)
                {
                    ...
                    faDocumentParams.Code = intDocEntry;
                    AssetDocument AssetDocument = AssetService.Get(faDocumentParams);

                    AssetDocument.Reference = strReference;
            ...
            catch (Exception ex)
            {
                ...
                SystemFunction.transHandler("Import", "", GlobalVariable.intObjType.ToString(), "", "", "", dteStart, "E", GlobalVariable.intErrNum.ToString(), GlobalVariable.strErrMsg);
